import React, { useEffect, useState } from 'react';
import axios from 'axios';
import { backendUrl } from '../App';
import { toast } from 'react-toastify';

const ArtistList = ({ token }) => {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  const currency = '₹';

  // Get artistId from localStorage
  const artistUser = localStorage.getItem('artistUser');
  const artistId = artistUser ? JSON.parse(artistUser).id : '';

  useEffect(() => {
    const fetchArtProducts = async () => {
      setLoading(true);
      try {
        const artistRes = await axios.get(`${backendUrl}/api/artists/${artistId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const artArray = artistRes.data.artist.art || [];

        if (artArray.length === 0) {
          setProducts([]);
          setLoading(false);
          return;
        }

        const productRes = await axios.post(
          `${backendUrl}/api/product/by-ids`,
          { ids: artArray },
          { headers: { Authorization: `Bearer ${token}` } }
        );
        setProducts(productRes.data.products || []);
      } catch (error) {
        toast.error('Failed to fetch artworks.');
        setProducts([]);
      }
      setLoading(false);
    };

    if (artistId && token) fetchArtProducts();
  }, [artistId, token]);

  return (
    <div className="flex flex-col w-full items-start gap-4 p-4 bg-gray-100 rounded-lg shadow-md">
      <p className="text-lg font-semibold text-gray-700 mb-2">My Artworks</p>
      <div className="flex flex-col gap-2 w-full">
        {/* Desktop Table header */}
        <div className="hidden md:grid grid-cols-[1fr_3fr_1fr_1fr] items-center py-2 px-4 border bg-gray-200 text-sm font-semibold text-gray-700 rounded-lg">
          <b>Image</b>
          <b>Name</b>
          <b>Category</b>
          <b>Price</b>
        </div>
        {/* Table body for desktop */}
        <div className="hidden md:flex flex-col gap-2 w-full">
          {loading ? (
            <div className="py-4 text-gray-500">Loading...</div>
          ) : products.length === 0 ? (
            <div className="py-4 text-gray-500">No artworks found.</div>
          ) : (
            products.map((item) => (
              <div
                key={item._id}
                className="grid grid-cols-1 md:grid-cols-[1fr_3fr_1fr_1fr] items-center py-2 px-4 border bg-white text-sm rounded-lg shadow-sm hover:shadow-md transition duration-300"
              >
                {/* Image */}
                <div className="flex justify-center md:block">
                  <img
                    src={item.image && item.image[0] ? item.image[0] : ''}
                    alt={item.name}
                    className="w-16 h-16 object-cover rounded shadow"
                  />
                </div>
                {/* Name */}
                <p className="font-medium text-gray-700">{item.name}</p>
                {/* Category */}
                <p className="text-gray-600">{item.category}</p>
                {/* Price */}
                <p className="text-gray-800 font-semibold">
                  {currency}
                  {item.price}
                </p>
              </div>
            ))
          )}
        </div>
        {/* Mobile cards */}
        <div className="md:hidden flex flex-col gap-4 w-full">
          {loading ? (
            <div className="py-4 text-gray-500">Loading...</div>
          ) : products.length === 0 ? (
            <div className="py-4 text-gray-500">No artworks found.</div>
          ) : (
            products.map((item) => (
              <div
                key={item._id}
                className="flex gap-4 items-center border bg-white rounded-lg shadow-sm p-3"
              >
                <img
                  src={item.image && item.image[0] ? item.image[0] : ''}
                  alt={item.name}
                  className="w-20 h-20 object-cover rounded shadow"
                />
                <div className="flex flex-col gap-1">
                  <span className="font-semibold text-gray-800">{item.name}</span>
                  <span className="text-gray-600 text-xs">{item.category}</span>
                  <span className="text-gray-800 font-bold text-sm">
                    {currency}
                    {item.price}
                  </span>
                </div>
              </div>
            ))
          )}
        </div>
      </div>
    </div>
  );
};

export default ArtistList;